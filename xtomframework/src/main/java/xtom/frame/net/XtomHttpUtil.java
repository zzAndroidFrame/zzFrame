...
public class XtomHttpUtil {
	...
	public static JSONObject sendPOSTWithFilesForJSONObject(String path,
		...
		return XtomJsonUtil.toJsonObject(sendPOSTWithFilesForString(path,
		...
		for (Map.Entry<String, String> entry : files.entrySet()) {
			...
			try {
				...
				dos.writeBytes("Content-Disposition: form-data; " + "name=\""
						...
						+ "\"" + END);
				XtomLogger.d(TAG, "The file path is \n" + entry.getValue());
				String filetype = XtomFileTypeUtil.getFileTypeByPath(entry
						.getValue());// 获取文件类型
				XtomLogger.d(TAG, "The file type is " + filetype);
	...
	private static void writeParams(String path,DataOutputStream dos,
		...
		if (params != null && !params.isEmpty()) {
			...
			if(XtomConfig.DIGITAL_CHECK){  //开启数字签名
				String datetime = XtomTimeUtil.getCurrentTime("yyyy-MM-dd HH:mm:ss");
				data.append("&").append("datetime=").append(datetime).append("&");
				String[] tempPath = path.split("/");
				String sign = Md5Util.getMd5(XtomConfig.DATAKEY+"|"+datetime+"|"+tempPath[tempPath.length-1]);
	 ...
	 * @throws DataParseException
	 */
	public static JSONObject sendPOSTForJSONObject(String path,
			HashMap<String, String> params, String encoding)
			throws DataParseException, HttpException {
		return XtomJsonUtil.toJsonObject(sendPOSTForString(path, params,
				encoding));
	}
	...
	public static String sendPOSTForString(String path,
		...
		try {
			...
			conn.setConnectTimeout(XtomConfig.TIMEOUT_CONNECT_HTTP);
			conn.setReadTimeout(XtomConfig.TIMEOUT_READ_HTTP);
			conn.setRequestMethod("POST");
			conn.setDoOutput(true);
			...
			XtomLogger.d(TAG, "The responsecode is " + code);
