...
public class XtomWindowSize {
	...
	public static void get(Context context) {
		if (height == 0 || width == 0) {
			...
			XtomLogger.d(TAG, "height=" + height + " width=" + width
					+ " statusBarHeight=" + statusBarHeight);
			if (width != 0) {
				XtomSharedPreferencesUtil.save(ac, "windowWidth",
						((Integer) width).toString());
				XtomSharedPreferencesUtil.save(ac, "windowHeight",
						((Integer) height).toString());
				XtomSharedPreferencesUtil.save(ac, "windowStatusBarHeight",
