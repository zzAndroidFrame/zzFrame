 ...
 * 图片工具类
 */
public class XtomImageUtil {
	private static final String TAG = "XtomImageUtil";
	...
	public static void deletePicInExternalMemoryCache(Context context,
		...
		XtomFileUtil.deleteFile(filepath);
	}
	...
	public static boolean savePicToExternalMemoryCache(Bitmap bitmap,
		...
		if (file == null)
			return false;
		String path = file.getPath();
		String folder = path.replace(XtomFileUtil.getExternalMemoryPath(), "")
				+ "/images/";
	...
	public static boolean savePicToExternalMemory(Bitmap bitmap, String folder,
			String name) {
		String path = XtomFileUtil.getExternalMemoryPath();
	...
	public static String compressPictureWithSaveDir(String path, int reheight,
		...
		if (!dir.exists())
			dir.mkdirs();
		// 保存
		String savepath = savedir + XtomBaseUtil.getFileName() + "_"
		...
		bitmap.recycle();
		System.gc();
		XtomLogger.d(TAG, "The new picture's path is " + savepath);
		return savepath;
	}
	...
	public static String compressPictureDepthWithSaveDir(String path, int reheight,
			int rewidth, int quality, String savedir, Context context)
			throws IOException {
		Bitmap bitmap = XtomImageUtil.getLocPicByDBYS(path, reheight, rewidth);
		File dir = new File(savedir);
		if (!dir.exists())
			dir.mkdirs();
		// 保存
		...
		if(quality >= 0 && quality <= 100){
			...
			options =  XtomImageUtil.QUALITY;
		}
		bitmap.compress(CompressFormat.JPEG, options, bos);
		while (bos.toByteArray().length / 1024 > XtomConfig.MAX_IMAGE_SIZE) {
			bos.reset();// 置为空
			// 每次都减少10
			options -= 10;
			// 压缩options%
	...
	public static String compressPicture(String path, int reheight,
		...
		Bitmap bitmap = getLocPicByDBYS(path, reheight, rewidth);
		// 设置压缩图片保存目录
		String savedir = XtomFileUtil.getTempFileDir(context);
		File dir = new File(savedir);
		if (!dir.exists())
			dir.mkdir();
		// 保存
	...
	public static Bitmap getLocPicByWidthorHeight(String path, int want,
		...
		XtomLogger.d(TAG, "The picture path is " + path);
		XtomLogger.d(TAG, "The picture inSampleSize is " + inSampleSize);
		XtomLogger.d(TAG, "The picture scale is " + scale);
		XtomLogger.d(TAG, "The picture degree is " + getPictureDegree(path));
	...
	public static Bitmap getLocalPicture(String path, int wantHeight,
		...
		try {
			...
			XtomLogger.d(TAG, "The picture path is " + path);
			XtomLogger.d(TAG, "The picture inSampleSize is " + inSampleSize);
			XtomLogger.d(TAG, "The picture scale is " + scale_w + "-----"
					+ scale_h);
			XtomLogger
