...
public class XtomRefreshLoadmoreLayout extends ViewGroup {
	...
	public XtomRefreshLoadmoreLayout(Context context, AttributeSet attrs) {
		super(context, attrs);
		init(context, attrs);
	}
	...
	public void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
		...
		if (count != 1 && !isInEditMode()) {
			throw new IllegalStateException(
					"XtomRefreshLoadmoreLayout must host only one direct child");
		}
	 ...
	 * 
	 * @param speedBeforeEnable
	 *            (0-1)默认为0.6,超出范围的视为默认值
	 */
	 ...
	 * 
	 * @param speedAfterEnable
	 *            (0-1)默认为0.3,超出范围的视为默认值
	 */
